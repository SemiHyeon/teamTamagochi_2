...
public class TamaManager {
    ...
    public void feed(){
        ...
        // 만약 포만감이 10이상인 경우, 일정 확률로 Tamagochi의 dieByEating() 메소드 호출. 즉 배부른 상태에서 밥을 먹이면 일정 확률로 죽음.
        if(tama.getSatiety() >= 10) {
            if(Math.random() < 0.4) {
                tombstones.add(tama.dieByEating("배부른 상태에서 먹다가 체해서(운 없어서) 죽음"));
            }
        }
        ...
        if(tama.getSatiety() >= 15){
            tombstones.add(tama.dieByEating("죽을 때까지 먹다가 배터져 죽음"));
        }
    }
    public void sleep(){
        Random random= new Random();
    ...
    public void gettingHungry(){
        ...
        if(tama.getSatiety() <= 0){
            tombstones.add(tama.dieByEating("배고파서 죽음"));
        }
    }
    public void gettingSleepy() {
        ...
        if (tama.getFatigue() >= 10) {
            if (tama.getFatigue() == 15) {
                tombstones.add(tama.dieBySleeping("피곤에 찌들어 죽음"));
